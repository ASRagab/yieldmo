object QuestionOne extends App {
  ...
  println("Test case: ")
  mergeGaps(segments).foreach(println)
  println("--------------")
  
  println("Empty List: ")
  //should work with empty List
  mergeGaps(List[Segment]()).foreach(println)
  println("--------------")
  
  println("Zero Gap Size: ")
  //gapSize = 0
  mergeGaps(segments, 0).foreach(println)
  println("--------------")
  ...
  mergeGaps(overlapped).foreach(println)
